...
<section class="onepage-hero">
  ...
  <p class="onepage-subtitle">从零上手到用顺手——一份写给普通人的 macOS 实战指南。覆盖软件推荐、系统操作、跨设备协作、效率工具、开发环境、维护排查、安全备份、换机迁移。</p>
</section>
...
<div class="quick-grid">
  ...
  <a href="#ecosystem"><div class="card-icon"><Share2 /></div><div class="card-body"><strong>跨设备协作</strong><span>接力、通用剪贴板、随航、AirDrop</span></div></a>
...
macOS 软件管理底座。命令行工具、图形 App、开发环境全靠它装。不会 Brew 的 Mac 用户，就像买了菜刀只会拆快递。详见 [5.2 Homebrew](#homebrew)。
...
免费开源的窗口管理。`Option + 方向键` 把窗口推到左半屏、右半屏、上半屏、居中、最大化。从 Windows 过来的人装上立刻就有家的感觉。进阶可上 Rectangle Pro，支持布局记忆和快捷手势。
...
菜单栏系统监控，CPU、GPU、内存、网速、电池、温度都能看。比一堆国产管家更像工具。免费开源。
...
**画图与设计（按需）**：[Krita](https://krita.org/)（绘画/插画，开源）、[GIMP](https://www.gimp.org/)（图片编辑，开源，体验硬核）、[Inkscape](https://github.com/inkscape/inkscape)（矢量图/SVG，开源）、[Blender](https://www.blender.org/)（3D，开源，功能离谱地强）。一般人用不上，一旦需要就很值。

### 📝 知识与办公

**[Obsidian](https://obsidian.md/)** ★★★★★ · 日常主力 · [Web Clipper](https://obsidian.md/clipper)

本地优先的笔记软件。所有数据存在本地，文件就是纯 Markdown，不怕被平台锁死。插件生态丰富。官方 Web Clipper 能把网页、高亮存进笔记库。已有知识库就别为了"开源洁癖"硬迁移，搬砖自虐没必要。
...
从 Windows 切到 Mac，最大的障碍不是操作，是概念。下面这张表覆盖了 90% 的困惑：
...
::: tip 效率建议
窗口管理用 Rectangle 比 macOS 自带分屏更快。`Option + 方向键` 左半屏、右半屏、居中，比长按绿色按钮快十倍。
:::
...
Mac 触控板是笔记本电脑里最好的，没有之一。但默认设置没有全开。
